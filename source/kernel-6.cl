...
__kernel void calc_speed(__global float3 *gpu_pos, __global float3 *gpu_speed, __global float3 *gpu_dist)
{
	...
	if (i == j)
	{
		...
		gpu_dist[i].s0 += distance; // gemiddelde afstand gewoon allemaal optellen en op het einde delen door #
        if(gpu_dist[i].s1 < distance) {
			gpu_dist[i].s1 = distance; // maximum afstand
		}
		if(gpu_dist[i].s2 > distance) {
			gpu_dist[i].s2 = distance; // minimum afstand
		}
}
//***************kernelcode voor atomische speedberekening******************************
// Niet nodig want geen race conditie, maar eens kijken of deze atomische operaties dan invloed hebben op de snelheid
__kernel void calc_speed_a(__global float3 *gpu_pos, __global float3_ *gpu_speed, __global float3 *gpu_dist) //meegeven float3_ ipv float3! Geen conversie nodig hiervoor, zie union boven
{
	const int i = get_global_id(0); //zie boven
	...
	if (i == j)
	{
		...
		// gigantische race condities! Beter eerst alles in een buffer per lichaam opslaan en samenvoegen apart doen nbody-7)
		gpu_dist[i].s0 += distance; // gemiddelde afstand gewoon allemaal optellen en op het einde delen door #
        if(gpu_dist[i].s1 < distance) {
			gpu_dist[i].s1 = distance; // maximum afstand
		}
